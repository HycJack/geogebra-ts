...
export class GeoElementBase implements CoreGeoElement, ConstructionElement {
  ...
  constructor(construction: Construction) {
    ...
    this.constructionIndex = Math.floor(Math.random() * 1000000);
  }
...
export class GeoPoint extends GeoElementBase implements Pathable {
  ...
  private pathParam: PathParameter = { t: 0 };
  ...
  getPathParameter(): PathParameter {
    if (this.path) {
      this.pathParam = this.path.getPathParameter(this.getX(), this.getY());
    }
  ...
  setPathParameter(param: PathParameter): void {
    ...
    if (this.path) {
      const pos = this.path.getPointOnPath(param.t);
