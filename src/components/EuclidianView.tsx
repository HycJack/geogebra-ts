...
import { ScreenPoint, WorldPoint, GeoElement } from '../types';
import { CanvasRenderer } from '../common/rendering/CanvasRenderer';
import { SnapManagerImpl, SnapTarget, DEFAULT_SNAP_CONFIG } from '../common/snapping';
...
export const EuclidianView = forwardRef<EuclidianViewRef, EuclidianViewProps>(function EuclidianView(
  ...
  const { state, dispatch, updatePointPosition, selectElements, setHovered, startDrag, endDrag, setPreview, addPoint, addLineFromPoints, addSegment, addCircle, addPolygon, deleteElement } = useGeoGebra();
  ...
  const handlePointerDown = useCallback(
      ...
      switch (state.interaction.mode) {
        ...
        case 'line': {
          if (pendingPoints.length === 0) {
            ...
            addLineFromPoints(
              state.construction.elements.get(p1Id)?.type === 'point' ? (state.construction.elements.get(p1Id) as any).x : pendingPoints[0].x,
              state.construction.elements.get(p1Id)?.type === 'point' ? (state.construction.elements.get(p1Id) as any).y : pendingPoints[0].y,
              state.construction.elements.get(p2Id)?.type === 'point' ? (state.construction.elements.get(p2Id) as any).x : worldPos.x,
              state.construction.elements.get(p2Id)?.type === 'point' ? (state.construction.elements.get(p2Id) as any).y : worldPos.y
            );
            setPendingPoints([]);
            setPreviewData(null);
          }
    ...
    [state.interaction.mode, pendingPoints, getMousePosition, screenToWorld, hitTest, selectElements, startDrag, addLineFromPoints, getSnappedPosition, addPoint, state.construction.elements]
  );
